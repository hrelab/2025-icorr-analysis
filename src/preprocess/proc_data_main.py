...
def process_data(args):
    ...
    for par in demographicDict.keys():
        ...
        for con in ["01", "02"]:
            for x in range(1, 9):
                if not os.path.exists(f"processed_data/{par}/{con}/"):
                    os.makedirs(f"processed_data/{par}/{con}")
                act = f"0{x}"
                ...
                tempForce.to_csv(f"processed_data/{par}/{con}/processed-force-{act}.csv")
                tempGame.to_csv(f"processed_data/{par}/{con}/processed-game-{act}.csv")
        ...
        for e in emgs:
            ...
            # Save the EMG data
            temp.to_csv(f"processed_data/{par}/{condition}/processed-emg-{activity}.csv")
