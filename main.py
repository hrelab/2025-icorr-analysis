...
def make_plot_for_certain_muscle(subject_data, condition_data, condition_id, colors):
    for muscle in [f"{column}_C{condition_id}" for column in subject_data.get_columns()[3:]]:
        ...
        plot_and_save(merged_data, f"Muscle {muscle} | Condition {condition_id}", f"../test_data/plot_2/{muscle}", "Activity", colors)


def make_emg_area_under_curve_on_different_activities_plots(subject_data, colors):
    condition_1_values = []
    condition_2_values = []
    ...
    make_plot_for_certain_muscle(subject_data, condition_1_values, "01", colors)
    make_plot_for_certain_muscle(subject_data, condition_2_values, "02", colors)

def make_emg_area_under_curve_on_different_conditions_plots(subject_data, colors):
    for i, conditions in enumerate(subject_data.get_clipped_activity_pairs()):
        ...
        plot_and_save(merged_data, "", f"../test_data/plot_1/activity_{subject_data.get_activity_id(2 * i)}", "Muscle", colors, ["Condition A", "Condition B"], True)

def make_emg_area_under_curve_on_condition_one_AD_EX_4Act_plots(subject_data : MurderWallDetective, colors):
    condition_1_values = []
    count = 0
    for i, conditions in enumerate(subject_data.get_clipped_activity_pairs()):
    ...
    for i, frame in enumerate(condition_1_values):
        temp = pd.concat([frame["AD_C01"], frame["EX_C01"]], axis=1)
        temp.columns = [f"AD_{i}", f"A{i}"]
        merged_data = pd.concat([merged_data, temp], axis=1)
    merged_data.to_csv(f"../test_data/plot_2/activity_01_02_03_04.csv")
    plot_and_save(merged_data, f"Muscle Activation over 4 Activities", f"../test_data/plot_2/activity_01_02_03_04", "Muscle", colors, ["AD", "EX"], True)
...
def main():
    ...
    make_emg_area_under_curve_on_condition_one_AD_EX_4Act_plots(subject_data, colors)
